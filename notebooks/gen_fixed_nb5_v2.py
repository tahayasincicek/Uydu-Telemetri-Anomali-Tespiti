...
**Amaç:** Etiket gerektirmeyen yöntemlerle anomali tespiti yapmak. Gözetimsiz modeller sadece Normal verilerle eğitilir; anormal örnekleri dışarıda bırakma prensibiyle çalışır.
...
Gözetimsiz modellerin en iyi öğrenme yöntemi: **Sadece Normal verilerle eğitim**. Eşik değeri (threshold) ise Validation seti üzerinde F1 skoru optimize edilerek belirlenir.""")
...
# Aynı split NB4 ile tutarlı olmalı
...
# Sadece Normal veriler (eğitim)
normal_mask = (y_train == 0).values
X_normal_train = X_train_s[normal_mask]

print(f"Eğitim (Normal): {X_normal_train.shape}")
print(f"Validation: {X_val_s.shape}")
print(f"Test: {X_test_s.shape}")

# Threshold optimizer
def find_best_threshold(scores, y_true, percentiles=range(75, 100)):
    best_f1, best_t = 0, np.percentile(scores, 95)
...
code("""iso = IsolationForest(n_estimators=300, contamination=0.10, random_state=42, n_jobs=-1)
iso.fit(X_normal_train)

scores_val = -iso.score_samples(X_val_s)
iso_thresh, val_f1 = find_best_threshold(scores_val, y_val.values)
print(f"Optimal Threshold: {iso_thresh:.4f} (Val F1: {val_f1:.4f})")
...
code("""ocsvm = OneClassSVM(kernel='rbf', gamma='scale', nu=0.10)
ocsvm.fit(X_normal_train)

scores_val = -ocsvm.decision_function(X_val_s)
ocsvm_thresh, val_f1 = find_best_threshold(scores_val, y_val.values)
...
code("""km = KMeans(n_clusters=5, random_state=42, n_init='auto')
km.fit(X_normal_train)

scores_val = np.min(km.transform(X_val_s), axis=1)
km_thresh, val_f1 = find_best_threshold(scores_val, y_val.values)
scores_test = np.min(km.transform(X_test_s), axis=1)
y_pred = (scores_test > km_thresh).astype(int)
print(classification_report(y_test, y_pred, target_names=['Normal', 'Anomali']))
print(f"AUC: {roc_auc_score(y_test, scores_test):.4f}")

# 3D PCA Görselleştirmesi
from sklearn.decomposition import PCA
pca = PCA(n_components=3)
X_test_pca = pca.fit_transform(X_test_s)
fig = px.scatter_3d(x=X_test_pca[:, 0], y=X_test_pca[:, 1], z=X_test_pca[:, 2],
                    color=y_test.astype(str), color_discrete_map={'0': 'blue', '1': 'red'},
                    opacity=0.6, title="K-Means: Anomalilerin 3D PCA Uzayında Ayrışması")
fig.update_layout(template='plotly_dark', margin=dict(l=0, r=0, b=0, t=30))
fig.show()""")

md("""---
## 🔍 Bölüm 4: Local Outlier Factor (LOF)""")

code("""lof = LocalOutlierFactor(n_neighbors=20, novelty=True)
lof.fit(X_normal_train)

scores_val = -lof.score_samples(X_val_s)
lof_thresh, val_f1 = find_best_threshold(scores_val, y_val.values)
...
history = ae.fit(X_normal_train, X_normal_train,
...
plt.title('Autoencoder Loss Eğrisi'); plt.legend()
plt.show()

recon_val = ae.predict(X_val_s, verbose=0)
scores_val = np.mean(np.power(X_val_s - recon_val, 2), axis=1)
ae_thresh, val_f1 = find_best_threshold(scores_val, y_val.values)
...
plt.xlim([0, np.percentile(scores_test, 98)])
plt.legend()
plt.show()""")

md("""---
## 🤝 Bölüm 6: Ensemble ROC Karşılaştırması""")

code("""plt.figure(figsize=(10, 8))
# Her unsupervised model için ROC çiz
models_scores = {
...
plt.title('Gözetimsiz Modeller ROC Karşılaştırması')
plt.legend(loc="lower right")
plt.show()""")

md("""---
## 💾 Bölüm 7: Model Kaydetme""")
